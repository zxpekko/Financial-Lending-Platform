...
public class LendReturnServiceImpl extends ServiceImpl<LendReturnMapper, LendReturn> implements LendReturnService {
    ...
    @Override
    public List<LendReturn> getListByLendId(Long lendId) {
    ...
    public void notify(Map<String, Object> paramMap) {
        ...
        boolean result = transFlowService.judgeExsit(agentBatchNo);
        if(result){
            log.warn("幂等性返回");
            return;
        }
...
//        UpdateWrapper<LendReturn> lendReturnUpdateWrapper = new UpdateWrapper<>();
        ...
        //获取回款明细
        List<LendItemReturn> lendItemReturnList = lendItemReturnService.getLendItemReturnByLendReutrnId(lendReturn.getId());
        lendItemReturnList.forEach(item -> {
            ...
            BigDecimal realAmount = lendItem.getRealAmount();
            lendItem.setRealAmount(realAmount.add(item.getInterest()));
